...
if (workspace=="office") {
  library(RSQLite,quietly=TRUE,lib.loc = file.path("Y:","AG_Baumgrass","AG-PRI","R","R-3.5.2","library"))
  library(tcltk2,quietly=TRUE,lib.loc = file.path("Y:","AG_Baumgrass","AG-PRI","R","R-3.5.2","library"))
  library(R.devices,quietly=TRUE,lib.loc = file.path("Y:","AG_Baumgrass","AG-PRI","R","R-3.5.2","library"))
} else {
  library(RSQLite,quietly=TRUE)#,lib.loc = file.path("","home","yhoang","R","library"))
  library(tcltk2,quietly=TRUE)#,lib.loc = file.path("","home","yhoang","R","library"))
  library(R.devices,quietly=TRUE)#,lib.loc = file.path("","home","yhoang","R","library"))
#used only to detect number of cores on machine
#library(parallel,quietly=TRUE)
#package used for parallele computing
...
if (workspace=="office") {
	...
	source(file.path("Y:","AG_Baumgrass","AG-PRI","PRIanalyzer","tcl","libbwidget.r"))
...
} else if (workspace=="YH") {
	setwd(file.path("","scratch","drfz","R"))
	fcs$db.path=file.path("","data","databases")
	fcs$db.name="SG_20161030_HumanCytokines_CD4mem.sqlite3"
	fcs$db.name="SG_20170517_NZBxW-TconPD1IFNgReceptors.sqlite3"
	fcs$db.name="SG_20170821_NZBxW-SubsetDistinction.sqlite3"
	#fcs$db.name="SG_20170821_NZBxW-TCellSubstets.sqlite3"
	fcs$db.name="SG_20170529_NZBxWIFNg+PD1+Signaling_old.sqlite3"
	...
	fcs$db.name="TL_20160721_NZBxW-Kinetics.sqlite3"
	fcs$db.name="AR_20170817_Spitzer_bloodsamples_day3.sqlite3"
	#fcs$db.name="AM_20180312_MS-exp-5-CIS13-CIS14-CIS15-CIS16-HC09-HC10-HC11-HC12_CD4+.sqlite3"
	#fcs$db.name="AR_20170817_Spitzer_spleensamples.sqlite3"
	#fcs$db.name="AR_20170907_Spitzer_lymphnodesamples.sqlite3"
	fcs$db.name="SG_20170915_NZBxW-TSubsets.sqlite3"
	fcs$db.name="SG_20171220_ThSubsets-NZBxW.sqlite3"
	...
	#fcs$png.file=file.path("tcl","tmp.png")
	#fcs$template.file=file.path("tcl","template.png")
}  else if (workspace=="Praktika") {
	setwd(file.path("Y:","AG_Baumgrass","Praktika2017_Grp1"))
	fcs$db.path=file.path("Y:","AG_Baumgrass","Praktika2017_Grp1","DB")
	fcs$db.name=""
	fcs$working=FALSE
	
	### load private library
	source(file.path("Y:","AG_Baumgrass","Praktika2017_Grp1","tcl","libbwidget.r"))
	### set template location
	fcs$png.file=file.path("Y:","AG_Baumgrass","Praktika2017_Grp1","tcl","tmp.png")
	fcs$template.file=file.path("Y:","AG_Baumgrass","Praktika2017_Grp1","tcl","template.png")
}
...
### if changes in database needs to bmatrix::e done
...
fcs$GUImain <- function () {
	...
	this$max.nchar = this$max.nchar + 18
	...
	#this$recent.fileindex = this$current.filenum
	...
	len.chars = max(nchar(this$selected.vars))
	this$width.panel = 315 + len.chars
	#this$width.panel = (round(len.chars*50)+64)


	ttf_main = tkframe(this$tt)
	this$ttf_markers = tkframe(ttf_main)

	### create paneled windows
	# resize the window and move the panel separator with the mouse
	### select/deselect buttons
	ttfselButtons=tkframe(this$ttf_markers)#,width=this$width.panel)
	### set label frame select markers of interest
	btselAll=tkbutton(ttfselButtons,text="Select all",command=function() {
		for ( i in 1:len ) {
			tclvalue(this$cbVal[[i]])="1"
		}
	})
	btdeselAll=tkbutton(ttfselButtons,text="Deselect all",command=function() {
		for ( i in 1:len ) {
			tclvalue(this$cbVal[[i]])="0"
		}
	})
	tkgrid(btselAll,btdeselAll,padx=5,pady=2,sticky="we")
	#tkgrid(ttfselButtons,sticky="we")
	###

	### cutoff buttons
	#ttfselCutoffs = tkframe(this$ttf_markers)#,width=this$width.panel)
	ttfloadsave=tkframe(ttfselButtons)
	...
	tkgrid(btuseSavedCutoffs,btsaveCutoffs2all,padx=2,sticky="we")
	btresetCutoffs=tkbutton(ttfselButtons,text="Reset cutoffs",command=function() {this$refreshCutoffs(reset=TRUE)})
	tkgrid(ttfloadsave,btresetCutoffs,pady=10,padx=5,sticky="we")
	...
	#tkgrid(ttfselCutoffs,sticky="we",padx=5,pady=5)
	tkgrid(ttfselButtons,pady=5,sticky="we")
	...
	scrollFrame <- tkwidget(this$sw,"ScrollableFrame",height=765,width=this$width.panel)
	tcl(this$sw,"setwidget",scrollFrame)
	subfID <- tclvalue(tcl(scrollFrame,"getframe"))
	#subfID <- tkgetframe(scrollFrame)$ID
	...
	while (i < (len*4) ) {
		...
		cutoffperccb=tcl("checkbutton",paste(subfID,".",i+3,sep=""),variable=this$cbcutoffperc[[j]],text="in %")
	  	tkgrid(cutoffcb,cutofflabel,cutoffentry,cutoffperccb,sticky="w",pady=2)
	...
	### frame with file names and tabs
	ttf_tabs <- tkframe(ttf_main)
	
	ttf_filenames_asinh = tkframe(ttf_tabs)
  	ttf_filenames = tkframe(ttf_filenames_asinh)
	ttfchoosefile = tkframe(ttf_filenames,height=30)
	if (this$working) {
	...
	tkgrid(ttlfcomment,pady=5)

# asinh radiobuttons --------------------------------------------------------
	ttf_radioasinh = tkframe(ttf_filenames_asinh)
	tkgrid(tklabel(ttf_radioasinh,text="asinh cofactor"),sticky="e")
	...
	tk2tip(rb1,"asinh(x/cof)")
	tk2tip(rb2,"asinh(x/cof)")
	tk2tip(rb3,"asinh(x/cof)")
	tk2tip(rb4,"asinh(x/cof)")
	tk2tip(rb5,"asinh(x/cof)")
	tkgrid(tfradios)

	tkgrid(ttf_filenames, ttf_radioasinh,sticky="e",padx=2)
	
	tkgrid(ttf_filenames_asinh)
	...
	if(length(this$total.projects)>1) {
	  ...
	  if (!this$openProjectYet) {
	    # tkbindtext(this$tt, "<Button-1>", function(...) {
...
fcs$GUIdiploT <- function(tab) {
	...
	ttfcombo=tkframe(tab)
	# combobox: binSize
	this$binSizedi=ttkcombobox(ttfcombo,values=this$binSizes,width=this$max.nchar-10)
	tkset(this$binSizedi,this$binSizes[param$binSizesDiPos])
	# combobox: mincount cells
	this$minCountDi=ttkcombobox(ttfcombo,values=this$min.counts.diploT,width=this$max.nchar-10)
	...
	this$cbvar1di=ttkcombobox(ttfcombo,values=this$selected.vars,width=this$max.nchar-10)
	tkset(this$cbvar1di,this$selected.vars[param$currVarDi1])
	this$cbvar2di=ttkcombobox(ttfcombo,values=this$selected.vars,width=this$max.nchar-10)
	...
	tkgrid(ttfcombo,padx=5,pady=2,sticky="w")

	# plot button
	ttfbut=tkframe(tab)
	ttfbutleft=tkframe(ttfbut)
	btplot= tkbutton(ttfbutleft,text="Plot diploT",command=function() {this$dodiploT(mode="single")})
	tkgrid(btplot,sticky="snwe",padx=10,pady=10)
	#tkgrid(btplot,sticky="snwe",padx=10,pady=2)
	tkconfigure(btplot,height=2,width=50)
	ttfbutright=tkframe(tab)
	bthist=tkbutton(ttfbutright,text="Plot histograms",command=function(){this$plotHistograms(plotter="di",pdf=FALSE)})
	tkgrid(bthist,sticky="we",pady=2)
	tkgrid(ttfbutleft,ttfbutright,padx=10)
	tkgrid(ttfbut)
	...
	# X axis
	minvalXdi=tkentry(ttfrange,width=6,textvariable=this$vminvalX)
	maxvalXdi=tkentry(ttfrange,width=6,textvariable=this$vmaxvalX)
	tkgrid(tklabel(ttfrange,text="Range x-axis (min/max):"),minvalXdi,maxvalXdi,padx=7,pady=2,sticky="w")
	# dynamic range
	cbtdynrange=tkcheckbutton(ttfrange,variable=this$cbtdynRange,text="Dynamic MSI range")
	tkgrid(cbtdynrange,sticky="nws",pady=2,padx=15)
	tk2tip(cbtdynrange,"Uncheck for manual input")
	# second param
	minMSI=tkentry(ttfrange,width=4,textvariable=this$vminMSI)
	maxMSI=tkentry(ttfrange,width=4,textvariable=this$vmaxMSI)
	tkgrid(tklabel(ttfrange,text="Min MSI:"),minMSI,padx=5,pady=2,sticky="e")
	tkgrid(tklabel(ttfrange,text="Max MSI:"),maxMSI,padx=5,pady=2,sticky="e")
	tkgrid(ttfrange)
	# add checkbuttons for plot attributss
	...
	tk2tip(cbtaddfilename,"Check to add title text on every diploT.")
	cbtaddDate=tkcheckbutton(ttfgridnew,variable=this$cbtaddDate,text="Date")
	...
	tkgrid(btsavePDF,padx=5,pady=2)
	...
	#rb2=tkradiobutton(ttfradiosright1,variable=this$rbtrans,value="biex")
	tkgrid(rb1,tklabel(ttfradiosright1,text="asinh"),rb2,tklabel(ttfradiosright1,text="none"))
	tkgrid(tklabel(ttfradios,text="Transformation:"),ttfradiosright1,sticky="w",pady=5)
	tkbind(rb1, "<Button-1>", function() { this$changeFI.range(1) })
	#tkbind(rb2, "<Button-1>", function() { this$changeFI.range(2) })
	...
	tkgrid(tklabel(ttfradios,text="Statistical method:"),ttfradiosright2,sticky="nw",pady=2)
	#tk2tip(ttfradiosright2,"If cutoff is set, it only considers producing cells of feature 3.")
	tkgrid(ttfradios,padx=10,pady=2)
	...
	tkgrid(cbttrimming,sticky="w",pady=5)
	tkgrid(cbtshowMinBins,sticky="w")
	tkgrid(btremovdub,ttfcheckbuttons,padx=5,pady=5,sticky="we")
	tkgrid(ttfbuttons,sticky="w")
	tkgrid(ttlfFI,ttlfinfo,padx=5,pady=5,sticky="wens")
	tkgrid(ttfvals,pady=5)
	#####
	...
	tkgrid(btplot1,btplot2,sticky="snwe",padx=20,pady=10)
	...
	tkgrid(btplot3,btplot4,sticky="snwe",padx=20,pady=10)
	...
	tkgrid.configure(lfinfo,sticky="swe",pady=25,padx=10)  
}
...
fcs$GUItriploT <- function(tab){
	...
	this$minCountTri=ttkcombobox(ttfcombo,values=this$min.counts,width=this$max.nchar-10)
	tkset(this$minCountTri,this$min.counts[param$minCountTriPos])
	this$binSize=ttkcombobox(ttfcombo,values=this$binSizes,width=this$max.nchar-10)
	...
	this$cbvar1=ttkcombobox(ttfcombo,values=this$selected.vars,width=this$max.nchar-10)
	tkset(this$cbvar1,this$selected.vars[param$currVarTri1])
	this$cbvar2=ttkcombobox(ttfcombo,values=this$selected.vars,width=this$max.nchar-10)
	tkset(this$cbvar2,this$selected.vars[param$currVarTri2])
	this$cbvar3=ttkcombobox(ttfcombo,values=this$selected.vars,width=this$max.nchar-10)
	tkset(this$cbvar3,this$selected.vars[param$currVarTri3])
	# call comboboxleft
	tkgrid(tklabel(ttfcombo,text="binSize:"),this$binSize,sticky="e",padx=5)
	tkgrid(tklabel(ttfcombo,text="minCount:"),this$minCountTri,sticky="e",padx=5)
	tkgrid(tklabel(ttfcombo,text="Feature A:"),this$cbvar1,sticky="e",padx=5)
	tkgrid(tklabel(ttfcombo,text="Feature B:"),this$cbvar2,sticky="e",padx=5)
	tkgrid(tklabel(ttfcombo,text="Feature C:"),this$cbvar3,sticky="e",padx=5)
	tkgrid(ttfcombo,padx=25,pady=2,sticky="w")
	
	#### rect buttons "Plot"
	ttfbut=tkframe(tab)
	ttfbutleft=tkframe(ttfbut)
	btplot= tkbutton(ttfbutleft,text="Plot triploT",command=this$dotriploT)
	btplot2= tkbutton(ttfbutleft,text="Plot for all files",command=this$dotriploTfiles)
	yhbtplot2= tkbutton(ttfbutleft,text="Read Table",command=this$dotriploTtable)
	if (this$working) {
	  tkgrid(btplot,btplot2,yhbtplot2,sticky="snwe",padx=5,pady=3)
	} else {
	  tkgrid(btplot,btplot2,sticky="snwe",padx=5,pady=3)
	}
	tkconfigure(btplot,height=2,width=25)
	ttfbutright=tkframe(ttfbut)
	bthist=tkbutton(ttfbutright,text="Plot histograms",command=function(){this$plotHistograms(plotter="tri",pdf=FALSE)})
	#btdens=tkbutton(ttfbutright,text="Plot density",command=function(){this$plotDensities(plotter="tri",pdf=FALSE)})
	tkgrid(bthist,sticky="we")
	#tkgrid(btdens,sticky="we",pady=2)
	tkgrid(ttfbutleft,ttfbutright,padx=10)
	tkgrid(ttfbut)
...
# checkbox axes range --------------------------------------------------	
	ttfSetAxes = tkframe(ttlfFI)
	# X axis
	this$minvalX=tkentry(ttfSetAxes,width=6,textvariable=this$vminvalX)
	this$maxvalX=tkentry(ttfSetAxes,width=6,textvariable=this$vmaxvalX)
	tkgrid(tklabel(ttfSetAxes,text="Range x-axis (min/max):"),this$minvalX,this$maxvalX,padx=7,pady=1,sticky="e")
	# Y axis
	this$minvalY=tkentry(ttfSetAxes,width=6,textvariable=this$vminvalY)
	this$maxvalY=tkentry(ttfSetAxes,width=6,textvariable=this$vmaxvalY)
	tkgrid(tklabel(ttfSetAxes,text="Range y-axis (min/max):"),this$minvalY,this$maxvalY,padx=7,pady=1,sticky="e")
	tkgrid(ttfSetAxes)
# checkbox dynamic range --------------------------------------------------
	ttfcheckDyn = tkframe(ttlfFI)
	cbtdynrange=tkcheckbutton(ttfcheckDyn,variable=this$cbtdynRange,text="Dynamic MSI range")
	tkgrid(cbtdynrange,sticky="ews",padx=10)
	tk2tip(cbtdynrange,"Uncheck for manual input")
	this$minMSI=tkentry(ttfcheckDyn,width=4,textvariable=this$vminMSI)
	this$maxMSI=tkentry(ttfcheckDyn,width=4,textvariable=this$vmaxMSI)
	#tkgrid(tklabel(ttfcheckDyn,text="Plot range z-axis (min/max):"),this$minMSI,this$maxMSI,padx=7,pady=1,sticky="we")
	tkgrid(tklabel(ttfcheckDyn,text="Min MSI:"),this$minMSI,padx=5,pady=1,sticky="e")
	tkgrid(tklabel(ttfcheckDyn,text="Max MSI:"),this$maxMSI,padx=5,pady=1,sticky="e")
	tkgrid(ttfcheckDyn, sticky="w")
# checkbox show grid --------------------------------------------------
	...
	tkgrid(cbtshowgrid,cbtpercentage,cbtshowlegend,cbtaddDate,padx=10,sticky="we")
	tkgrid(ttfcheckPlot)
	# option panel
...
# radio buttons for transformation type -----------------------------------
	...
	tk2tip(cbtshowMinBins,"Show bins in pale color.")
	tkgrid(btremovdub,cbttrimming,cbtshowMinBins,padx=5,pady=2,sticky="we")
	tkgrid(ttfbuttons,sticky="w")
	...
	tkgrid(btOverviewXY,sticky="snwe",pady=3,padx=5)
	...
	tkgrid(btOverviewX,sticky="snwe",pady=3,padx=5)
	...
	tkgrid(btOverview,sticky="snwe",pady=3,padx=5)
	...
	tkgrid(btsavePNG,pady=2,sticky="we")
	tkgrid(btsavePDF,pady=2,sticky="we")
	tk2tip(btsavePDF,"Save as pdf")
	tk2tip(btsavePNG,"Save as jpg")
	tkgrid(ttfsave,padx=3)
	tkgrid(ttfrowcol,ttfgridsave,padx=5,pady=2)
	# set active window buttons
	#tkgrid(tkbutton(ttlfgraph,text="Set prev window as active",command=function(){dev.set(dev.prev())}),
	#		tkbutton(ttlfgraph,text="Set next window as active",command=function(){dev.set(dev.next())}),padx=5,pady=2)
	tkgrid(ttlfgraph,padx=5,pady=3)
	...
	tkgrid(butdef, cbtmanrect,padx=10,pady=2)
	...
	#ä#tkgrid(,padx=5,pady=3,sticky="n")
	...
	tkgrid(tkbutton(ttfaddInfo,text="Add rect info",command=function(){this$addRectInfo(setcex=1.0)}),pady=2)
...
fcs$GUIdataframe <- function(tab) {
	...
	this$treeDF <- tkwidget(this$pw2, "Tree",
		...
		width = 20, height =20,
		opencmd=function (...) { this$mytkopentree(this$treeDF,...) },
		closecmd=function (...) { this$mytkclosetree(this$treeDF,...)}
	)
	...
	tkpack(btn1,btn2,padx=5,pady=3,side="left")
...
fcs$GUIlog <- function(tab) {
	...
	text <- tk2text(ttfmain,width=90, height=40)
...
fcs$GUIpreprocMode <- function(mode) {
	...
	if (mode == 1) {
		...
		tkgrid(cbttrimming,cbtremovedub,sticky="w",pady=5,padx=10)
		...
		tkgrid(bthist,btautogate,btinteractgate,padx=20,pady=20)
		...
		for ( i in 1:quarter ) {
			...
			tkgrid(cb,tklabel(ttfcol1,text="cutoff: "),cutoffentry,padx=2,pady=5,sticky="w")
		}
		for ( j in (i+1):(i+quarter) ) {
			...
			tkgrid(cb,tklabel(ttfcol2,text="cutoff: "),cutoffentry,padx=2,pady=5,sticky="w")
		}
		for ( k in (j+1):(j+quarter) ) {
			...
			tkgrid(cb,tklabel(ttfcol3,text="cutoff: "),cutoffentry,padx=2,pady=5,sticky="w")
		}
		...
		for ( l in (k+1):len ) {
			...
			tkgrid(cb,tklabel(ttfcol4,text="cutoff: "),cutoffentry,padx=2,pady=5,sticky="w")
		}
		tkgrid(ttfcol1,ttfcol2,ttfcol3,ttfcol4,padx=20,pady=5,sticky="we")
		tkgrid(this$ttlfselMarker,padx=10)
		
		tkgrid(this$ttlfautogate,pady=5)
	} else if (mode == 2) {
		# create info panel
		...
		tkgrid(btnormalize,pady=20,sticky="snew")
		...
		tkgrid(btselAll,btdeselAll,padx=20,pady=5)
		tkgrid(ttfbuttons)
		...
		for ( i in 1:(floor(len/3)+(len%%3)) ) {
			...
			tkgrid(cb,padx=2,pady=5,sticky="w")
		}
		...
		tkgrid(ttfcol1,ttfcol2,ttfcol3,padx=20,pady=5,sticky="wen")
...
fcs$refreshPanel <- function() {
	...
	scrollFrame <- tkwidget(this$sw,"ScrollableFrame",height=770,width=this$width.panel)
	tcl(this$sw,"setwidget",scrollFrame)
	subfID <- tclvalue(tcl(scrollFrame,"getframe"))
	# label project name
	...
	while (i < (len*4) ) {
		...
		cutoffentry=tcl("entry",paste(subfID,".",i+2,sep=""),width=6,textvariable=this$vcutoffs[[j]])
		cutoffperccb=tcl("checkbutton",paste(subfID,".",i+3,sep=""),variable=this$cbcutoffperc[[j]],text="in %")
		tkgrid(cutoffcb,cutofflabel,cutoffentry,cutoffperccb,sticky="w",pady=3)
		i = i + 4
		j = j + 1
	}
}
...
fcs$bindiplot <- function(data,cutoffs,bin.pos,set.cex=1.1,set.cex.axes=1.0, totalcex=1.0) { 
	this=fcs
	...
	if (cutoffs[1]>0 & checkCALC != "freq") {
		...
		#my.calc.pos=cbind(my.calc,ncells=my.lengths$x)
		### get color steps and color "factors" my.calc.fac.pos
		#idx.lower=which(my.calc.pos$ncells<mincells)
	...
	if ( nrow(data) == 0 ) {
		...
		if ( checkCALC == "density") {
			...
			my.calc=aggregate(data[,1],by=list(fX),length)
		...
		if (!empty) {
			...
			##### bindiplot
				for ( i in idx ) {
					...
					rect(sub2,bin.pos,sub2+binSize,bin.pos+0.5,col=cols[my.calc$fac[i]],border="darkgrey",lwd=0.1)
			...
			for ( i in min(idx,idx.lower):max(idx,idx.lower) ) {
				...
				### if i is in idx.lower 
				# then there were not enough cells in the bin
				# so plot the bin in pale
				if (any(i==idx.lower)) {
					#rect(sub2,y,sub2+binSize,y+0.5,col="lightgrey",border="white",lwd=0.2)
					if (checkSHOWMINBIN =="1") {
						rect(sub2,bin.pos,sub2+binSize,bin.pos+0.5,
						col=cols.pale[my.calc$fac[i]],border="white",lwd=3)
					}
				} else {
					rect(sub2,bin.pos,sub2+binSize,bin.pos+0.5,
						col=cols[my.calc$fac[i]],border="darkgrey",lwd=0.05)
					if (my.calc$ncells[i]>max.cells) max.cells=my.calc$ncells[i]   
					if (my.calc$ncells[i]<min.cells) min.cells=my.calc$ncells[i]   
			...
			### plot gate line and cell frequency
			if ( cutoffs[2] != 0 & checkCALC == "freq") {
				...
				cells.right = 100*(nrow(data.right))/data.lengths

				### and plot the percentages of cells left and right from cutoff x
				text(x=plot.xmin-binSize,y=bin.pos+0.61+this$legend.space.di,label=sprintf("%0.1f%%",cells.left),cex=0.43*set.cex,pos=4)
				text(x=plot.xmax+binSize+0.2,y=bin.pos+0.61+this$legend.space.di,label=sprintf("%0.1f%%",cells.right),cex=0.43*set.cex,pos=2)
				...
				text(x=plot.xmin+binSize,y=bin.pos+0.61+this$legend.space.di,label=sprintf("%0.1f%%",prod.left),cex=0.43*set.cex,col="red",pos=2)
				text(x=plot.xmax+binSize-0.2,y=bin.pos+0.61+this$legend.space.di,label=sprintf("%0.1f%%",prod.right),cex=0.43*set.cex,col="red",pos=4)
			}
			else if ( cutoffs[2] != 0 ) {
				segments(cutoffs[2], bin.pos-0.12, x1 = cutoffs[2], y1 = bin.pos+0.55)
				
				### get the perc of cells left and right from cutoff x
				data.left = data.old[which(data.old[,2]<cutoffs[2]),]
				data.right = data.old[which(data.old[,2]>=cutoffs[2]),]
				#cells.left = 100*(nrow(data[which(data[,2]<cutoffs[2]),])/nrow(data))
				cells.left = 100*(nrow(data.left))/data.lengths
				cells.right = 100*(nrow(data.right))/data.lengths
				...
				### display production percentage on each side of gate line
				if ( cutoffs[1] != 0 ) {
					...
					#if (this$working) printf("w: nrow(data)=%s nrow(data.left)=%s -->%s freq.prod=%0.1f%%/%0.1f%%",data.lengths,nrow(data.left),nrow(data.left[which(data.left[,1]<cutoffs[1]),]),prod.left,prod.right)

					text(x=plot.xmin+binSize,y=bin.pos+0.61+this$legend.space.di,label=sprintf("%0.1f%%",prod.left),cex=0.43*set.cex,col="chartreuse4",pos=2)
					text(x=plot.xmax+binSize-0.2,y=bin.pos+0.61+this$legend.space.di,label=sprintf("%0.1f%%",prod.right),cex=0.43*set.cex,col="chartreuse4",pos=4)

					#if (this$working) printf("w: cutoffs=%s  freq.prod=%0.1f%%/%0.1f%%",paste(cutoffs,collapse=" "),prod.left,prod.right)
				}
			}

			### print min/max value
			if (checkCALC == "density" | checkCALC == "freq" | checkCALC == "RSEM") print.minmax=sprintf("min/max=%0.0f/%0.0f",minX,maxX)
			else print.minmax = sprintf("min/max=%0.2f/%0.2f%s",minX,maxX,range)

			### print to the right of bins
			text(x=sub2+0.8,y=bin.pos,label=print.minmax,col=col.minmax,cex=0.43*set.cex*totalcex,pos=4,xpd=TRUE)
		}
	}

	### print calculation method and cutoff if available
	if (checkCALC == "freq") {
		print.label=sprintf("%s,cutoff=%s",checkCALC,cutoffs[1])
	} else if (cutoffs[1] > 0) {
		print.label=sprintf("%s(+),cutoff=%s",checkCALC,cutoffs[1])
	} else {
	...
	if (empty) {
		if (this$distepy=="1") text(x=xmax.val*2/3,y=bin.pos+0.2,label=print.label,cex=0.5*set.cex*totalcex,pos=4,col="red",xpd=TRUE)
		else text(x=this$current.diploT.x,y=bin.pos+0.33,label=print.label,cex=0.5*set.cex*totalcex,pos=4,col="red",xpd=TRUE)
	} else {
		text(x=sub2+0.8,y=bin.pos+0.33,label=print.label,cex=0.5*set.cex*totalcex,pos=4)
	}

	### print second feature on y axis horizontal
	if (checkADDFILE=="1") {
		title.text=as.character(tclvalue(tkget(this$title,"1.0","end-1c")))
		displayfile=this$shortenFilename(title.text)
		this$axis.label[bin.pos+1]=paste(displayfile,colnames(data)[1],sep=": ")
	} else {
		this$axis.label[bin.pos+1]=colnames(data)[1]
	}
	if ( all(checkCALC != c("density","SD","RSEM","SEM")) ) {
		text(x=plot.xmin,y=bin.pos+0.2,labels=this$axis.label[bin.pos+1],
			cex=0.5*set.cex,pos=2,mgp=set.mgp,xpd=TRUE)
	}
	...
	if ( my.LENGTHS ) {
		...
		if ( (!bg | density) & !png & this$plot.percentage) {
			if ( cutoffs[1] > 0 & cutoffs[2] > 0 & cutoffs[3] > 0 & !density) {
				...
				quadrant.percs <- cbind(displayfile,colnames(data)[1],colnames(data)[2],colnames(data)[3],checkCALC
				...
				write.table(quadrant.percs, sprintf("%s_PRIvis_metadata.csv",this$current.project), sep = ",", col.names = F, row.names = F, append = T)
				printf("Quadrant percs written in %s",sprintf("%s_PRIvis_metadata.csv",this$current.project))
...
fcs$getInfo <- function (file,vars) {
	...
	if (this$working) printf("w: do getInfo file=%s GATED=%s temp.num=%s",file,checkGATED,this$temp.num)
	...
	#if (this$working) printf("saved vars for gating: %s",vars)
	...
	#this$refreshDFtree(this$treetable)
}
...
fcs$saveCutoffsToAll <- function() {
  ...
  if (tclvalue(answer) == "yes") {
    ...
    new.staintable=cbind(this$current.staintable[,1:4]
...
fcs$changeFI.range <- function (mode) {
	...
	if ( mode == 1 ) {
		...
		#tkset(this$minCountTritetra,this$current.asinh$mincount)

		## change in triploT Overview
		#tkset(this$binSizeOverview,this$current.asinh$binSize)
		#tkset(this$minCountTriOverview,this$current.asinh$mincount)
		...
		## change in tetraploT
		#tkconfigure(this$binSizetetra,values=this$binSizes.biex)
		#tkset(this$binSizetetra,this$current.biex$binSize)
		#tkset(this$minCountTritetra,this$current.biex$mincount)

		## change in triploT Overview
		#tkconfigure(this$binSizeOverview,values=this$binSizes.biex)
		#tkset(this$binSizeOverview,this$current.biex$binSize)
		#tkset(this$minCountTriOverview,this$current.biex$mincount)
